...
export default function BookLoansPage() {
    ...
    const fetchBorrows = async () => {
      try {
        ...
        const url = statusFilter
          ? `${API_URL}/admin/borrows?status=${encodeURIComponent(statusFilter)}`
          : `${API_URL}/admin/borrows`;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setBorrowsData(data.borrows || []);
        showSuccess('Success', 'Table data refreshed');
      } else {
        showError('Error', 'Failed to refresh table data');
      }
    } catch (err) {
      console.error('Error refreshing borrows:', err);
      showError('Error', 'Error refreshing table data');
    } finally {
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Refresh borrows list with current filter
        const url = statusFilter
          ? `${API_URL}/admin/borrows?status=${encodeURIComponent(statusFilter)}`
          : `${API_URL}/admin/borrows`;
        const borrowsResponse = await fetch(url, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/admin/borrows/${id}/reject`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleRejectReturn = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/admin/borrows/${id}/reject-return`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        <>
          <div className="flex gap-4 mb-6 items-center">
            ...
            <button
              ...
              className="bg-white border-2 border-gray-300 text-gray-900 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed"
