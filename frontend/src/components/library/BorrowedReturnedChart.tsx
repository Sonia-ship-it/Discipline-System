...
export default function BorrowedReturnedChart({ 
  graphData, 
  currentFilter, 
  onFilterChange,
  isLoading 
}: BorrowedReturnedChartProps) {
  ...
  const filters = ['5days', 'month', 'year'];
  const displayFilter = currentFilter === '5days' ? 'weekly' : 
                        currentFilter === 'month' ? 'monthly' : 
                        currentFilter === 'year' ? 'yearly' : currentFilter;
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4 md:mb-8 gap-3 md:gap-4">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none bg-slate-900 text-white px-4 md:px-8 py-2 rounded-[5px] text-xs md:text-sm font-medium hover:bg-slate-800 transition-colors cursor-pointer pr-10"
          >
            <option value="5days">weekly</option>
            <option value="month">monthly</option>
            <option value="year">yearly</option>
          </select>
          <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-3 md:h-4 w-3 md:w-4 text-white pointer-events-none" />
        </div>
      </div>
