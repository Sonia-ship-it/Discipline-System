...
import { Bell, User, Menu, ChevronDown, LogOut, PanelLeft, Loader2 } from 'lucide-react';
...
export default function TopBar({
  ...
  // Library notification API uses a separate backend; avoid 401 loops with discipline JWT
  const { data: unreadData } = useUnreadCount(undefined, false);
  ...
  return (
    <div
      className={`sticky top-0 h-20 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-40 w-full min-w-0 overflow-visible shrink-0`}
      style={{ zIndex: 40 }}
    >
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50" style={{ zIndex: 9999 }}>
              ...
              <button
                ...
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
